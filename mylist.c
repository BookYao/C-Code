...
static void find_for_each_app(int appid, const struct list_head *head)
{
	struct list_head *pos;
	app_info *app;
...
void destroy_app_list(struct list_head *head)
{
	...
	{
		app = list_first_entry(head, app_info, app_info_node);
		list_del(&app->app_info_node);
		free(app);
	}
}
...
int main()
{
    ...
    /* start find */
    printf("\nStart find appid\n");
    find_for_each_app(1002, head);

    printf("Del Find node  app_info: \n");
    for_each_app(head);
